    ...
    function __construct()
    {
        ...
        foreach ($url as $key => $value) {
            ...
            if (!file_exists($cookieFile)) {
                ...
                while (!feof($file)) {
                    $this->conteudo .= fread($file, 1024);
                }
                fclose($file);

                $explodir = explode(chr(9), $this->conteudo);
            ...
            if (!empty($result)) {
                ...
                if ($key == 'cnpj') {
                    $this->imagem_cnpj = 'data:image/png;base64,' . base64_encode($result);
                } else if ($key == 'cpf') {
                    // pega token e captcha
                    $token_captcha = explode('@', $result);

                    $this->token_cpf = $token_captcha[0];
                    $this->imagem_cpf = 'data:image/png;base64,' . $token_captcha[1];    // esta imagem do captcha de CPF já está encodada base 64
                }
            }
        }
    }
